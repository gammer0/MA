...
执行层（`demo-app/`）是**独立于安全内核**的多Agent协作演示系统。它不包含在 `services/` 目录中，仅依赖 Agent SDK 通过 HTTP API 与权限网关交互。

### 核心职责

- 独立的演示项目，通过安全内核 API 进行身份验证和权限管控
- 凭证零硬编码：依赖运行时热注入（`POST /admin/keys`）或环境变量
- 同时展示 MCP 直接调用、A2A 编排调度两种模式
