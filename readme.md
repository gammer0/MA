...
1. 启动系统 → `docker compose up -d` + 批量注册
2. 正常委托 → 飞书文档助手 委托 企业数据 Agent 读取数据 → 放行 ✅
3. 越权拦截 → 外部检索 Agent 尝试访问企业数据 → 拒绝 ❌
4. 日志完整 → 管理页面输入 Task ID 查看完整审计日志

## 当前状态

- 安全内核三模块 (身份/权限/审计) 完备
- 3 Agent 协作演示 (Orchestrator + Searcher + Analyzer)
- 权限模型：交集视图 + 精确匹配 + deny优先
- 人工审批 + 自动降级双模式
- 任务审计列表 + 调用链可视化
- 9 单元测试全部通过
